...
class ARSessionManager {
  ...
  /// Hit-test a screen point (view pixels). Defaults to view center.
  /// Used by the "Place floor marker" button so placement does not depend on
  /// gesture delivery through Flutter PlatformViews.
  ///
  /// [allowInstantPlacement] defaults to false so height calibration waits for
  /// a real horizontal plane (white floor grid) instead of a 1.4m estimate.
  Future<List<ARHitTestResult>> hitTestScreen({
    double? x,
    double? y,
    double approxDistanceMeters = 1.4,
    bool allowInstantPlacement = false,
  }) async {
    try {
      final raw = await _channel.invokeMethod<List<dynamic>>('hitTestScreen', {
        if (x != null) 'x': x,
        if (y != null) 'y': y,
        'approxDistanceMeters': approxDistanceMeters,
        'allowInstantPlacement': allowInstantPlacement,
      });
      if (raw == null) return const [];
      return raw
          .map((e) => ARHitTestResult.fromJson(Map<String, dynamic>.from(e as Map)))
          .toList();
    } catch (e) {
      print('hitTestScreen error: $e');
      return const [];
    }
  }

  /// Samples several lower-screen points for a real horizontal plane.
  Future<List<ARHitTestResult>> hitTestFloor() async {
    try {
      final raw = await _channel.invokeMethod<List<dynamic>>('hitTestFloor');
      if (raw == null) return const [];
      return raw
          .map((e) => ARHitTestResult.fromJson(Map<String, dynamic>.from(e as Map)))
          .toList();
    } catch (e) {
      print('hitTestFloor error: $e');
      return const [];
    }
  }

  /// JPEG of the ARCore camera image (avoids PixelCopy hangs on some OEMs).
  Future<Uint8List?> captureCameraJpeg() async {
    try {
      return await _channel.invokeMethod<Uint8List>('captureCameraJpeg');
    } catch (e) {
      print('captureCameraJpeg error: $e');
      return null;
    }
  }
  ...
  Future<ImageProvider> snapshot() async {
    ...
    if (result == null || result.isEmpty) {
      throw Exception('AR snapshot returned empty');
    }
